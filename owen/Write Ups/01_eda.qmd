...
The following metric (R-hat) indicates if the Bayesian model is
converging. We would like values of smaller than 1.01 here, with values
less than 1 possible in a sample. 
```{r compute r-hat}
bayesplot::rhat(fit3_ave)
```
...
### Probability of Positive Winner Effects

For each of these players, we can use the posterior draws to
estimate the posterior probability that $\beta >0$, i.e
that there is a positive effect.

```{r estimate prob beta positive}

fit3_ave <- readRDS("../model_fits/init_model_n10.RDS")

# stan_draws <- fit3_ave$draws()
# 
# 
...
prob_positive <- function(stan_draws) {
  all_draws <- as_draws_df(stan_draws)
  ndraws <- nrow(all_draws)
  as_tibble(all_draws) %>% 
  select(starts_with("beta")) %>% 
...
We can then use this metric to compare how these winner effects change 
as we change both the length of the history and the type of games used.
