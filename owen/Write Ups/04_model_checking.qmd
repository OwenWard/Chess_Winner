...
which 675k games, ranging from 500 - 128k games per player.
...
users_1 <- unique(small_data_1$Username)

small_data_1$dttm <- as.POSIXct(paste(small_data_1$UTCDate,
                                      small_data_1$UTCTime),
                              format="%Y-%m-%d %H:%M:%S", tz = "UTC")
...
small_data_1 %>% 
  ...
  arrange(UTCDate, UTCTime, .by_group = TRUE) %>% 
  select(Username, dttm, UserELO) %>% 
  filter(Username %in% users_1[1:16]) %>% 
  # slice_sample(n = 100) %>% 
  ggplot(aes(dttm, UserELO, colour = as.factor(Username))) +
  geom_line() +
  labs(colour = "User", x = "Date") +
  geom_hline(yintercept = c(1700, 1900), 
             col = "red", linetype = "dashed", alpha = 0.5) +
  facet_wrap(~Username, scales = "free")
```
