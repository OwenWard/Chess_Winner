...
mcmc_hist(fit3_ave$draws(c("alpha", "mu2")))
...
have approximately a 54% chance of winning with white. Wikipedia says
the increase 
...
mcmc_hist(fit4$draws(c("delta", "mu3"))) +
  labs(title = "Effect of Previous Game")

mcmc_hist(fit4$draws(c("beta", "mu1"))) +
...
of opponents, many who play only a small number of games.
...
This would be done by only using the final game in each subsequent
window of size $n$, reducing the number of observations in 
the data by a factor of $n$ essentially. Initially it was
thought that this was being considered in the Deng et al. paper,
...
A valid concern here is the large correlation between the data used at
each step. For example, when using the previous 10 games as the history,
subsequent games will use an average with 9 of the 10 terms in the average being
identical. This leads to severe correlation.

One potential way to deal with this (mentioned by the authors of the Deng et al 
paper) is to test for this effect. One way to do this would be to consider
a permutation test. The results can be randomly permuted, which allows the
construction of an empirical set of estimates under the permutation.


This can be implemented for the simple initial model, considering
only games involving the 18 most frequent players. We randomly permute the 
results from the corresponding matches and refit the above model 100 times.
...
overall proportion of wins in the dataset?

```{r}

init_data

mean(init_data$focal_result)

## not that different to 1/(1 + exp(-mu_2)), 
## i.e just the proportion of games won by the focal player

## is it something similar for gamma1, larger number of games as white
## by the focal players?

mean(init_data$focal_white)
...
#### Checking Permuting within Players only

```{r read the data in}
perm_path <- here("owen", "cluster_scripts",
                  "perm_outputs", "perm_outputs_indiv")
...
perm_results %>% 
  ...
  geom_histogram() +
  labs(title = "Permutations of Mu_2",
       subtitle = "This is the mean of the player effects",
       x = "mu2") +
  geom_vline(data = orig %>% 
               filter(variable == "mu2"),
             aes(xintercept = mean), col = "red")
...
perm_results %>% 
  ...
  geom_histogram() +
  labs(title = "Permutations of gamma_1",
       subtitle = "True data appears similar",
       x = "gamma_1") +
  geom_vline(data = orig %>% 
               filter(variable == "gamma1"),
             aes(xintercept = mean), col = "red")

## effect of elo difference
...
## some of these still significant here, need to check it
