...
small_data <- lichess_data |>
  ...
  filter(
    Event == paste0("rated bullet game"),
    Variant == "Standard"
  ) |>
...
small_data <- lichess_data |>
  mutate(Event = tolower(Event)) |>
  filter(
    Event == paste0("rated ", time_control, " game"),
    Variant == "Standard"
  ) |>
  filter(TimeControl %in% c("60+0", "180+0")) |>
  distinct() # remove the duplicate rows if they exist
...
ggsave(
  plot = curr_dist,
  filename = here("Paper_WriteUp", "paper_figures", "response_to_comments", "curr_xij.png"), dpi = 1000,
  height = 4, width = 6
)
...
ggsave(
  plot = prop_dist,
  filename = here("Paper_WriteUp", "paper_figures", "response_to_comments", "prop_xij.png"), dpi = 1000,
  height = 4, width = 6
)
```
