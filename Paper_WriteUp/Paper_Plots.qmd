...
result |>
  ...
  summarise(
    med_session = median(session_length),
...
users <- c(
  "larrywheels", "SeanBambic", "dewang_007", # 1700-1900s
  "DrNykterstein", "alireza2003", "BlueGreensun"
) # gms
...
plot_data <- lichess_data |>
  ...
  mutate(
    ...
    Username = factor(Username, levels = c(
      "DrNykterstein (Magnus Carlsen)", "alireza2003 (Alireza Firouzja)",
      "BlueGreensun", "SeanBambic", "dewang_007", "larrywheels"
    ))
  ) |> # for order in legend
  ungroup() |>
...
# plotting
rating_plot <- ggplot(data = plot_data, 
                      mapping = aes(x = game, y = focal_rating,
                                    colour = Username)) +
  geom_line() +
...
log_time_diff_plot_gm <- ggplot(gm_time_diff_df, 
                                aes(x = as.numeric(time_diff))) +
  geom_histogram() +
  scale_x_continuous(
...
```{r figure 3}
# read in models
fit_17_19_bullet_prev <- readRDS(here("results_revision_may", "lichess1700-1900", "all_rated_bullet_model_n1.RDS"))
fit_20_22_bullet_prev <- readRDS(here("results_revision_may", "lichess2000-2200", "all_rated_bullet_model_n1.RDS"))
fit_23_25_bullet_prev <- readRDS(here("results_revision_may", "lichess2300-2500", "all_rated_bullet_model_n1.RDS"))
fit_gm_bullet_prev <- readRDS(here("results_revision_may", "lichessGrandmasters", "all_rated_bullet_model_n1.RDS"))

# read in users
bullet_17_19 <- readRDS(here("results_revision_may", "lichess1700-1900", "users_bullet.RDS"))
bullet_20_22 <- readRDS(here("results_revision_may", "lichess2000-2200", "users_bullet.RDS"))
bullet_23_25 <- readRDS(here("results_revision_may", "lichess2300-2500", "users_bullet.RDS"))
bullet_gm <- readRDS(here("results_revision_may", "lichessGrandmasters", "users_bullet.RDS"))
...
#| eval: false
gamma_ests <- fit_17_19_bullet_prev$summary(
  variables =
    c(
      "gamma1",
      "gamma2"
...
#| eval: false
gamma_ests_gm <- fit_gm_bullet_prev$summary(variables = c(
  "gamma1",
  "gamma2"
)) |>
  pull(mean)
...
winner_plot_17_19 <- player_pars |>
  ...
  stat_pointinterval() +
  labs(
    x = "Change in Win Probability", y = element_blank(),
...
indiv_plot_17_19 <- player_pars |>
  ...
  stat_pointinterval() +
  labs(
    x = "Estimated Player Effect", y = element_blank(),
    title = "1700-1900 Cohort"
  ) +
  geom_vline(aes(xintercept = 0), col = "red", linetype = "dashed") +
  theme_single_y()
...
model_path <- here("results_revision_may", "lichessGrandmasters")
...
init_data <- tidy_games |>
  ...
  dplyr::select(
    focal_user, focal_id, focal_white, WhiteElo, BlackElo, White, Black,
    focal_win_prop, elo_diff, focal_result, opp, opp_rating,
    focal_rating, UTCDateTime
  ) |>
  group_by(focal_id) |>
  mutate(
    time_diff = UTCDateTime - lag(UTCDateTime, default = NA), # default is to ensure first game is always start of a new session
  ...
  ) |> # if game played in same session, rolling mean over the past n games, removing standardization
...
focal_init <- data.frame(
  "Player" = users[focal_player_used],
...
for (i in 1:num_games) {
  ...
  for (j in 1:num_sims) {
    ...
    initstate <- tibble(
      Player = c(users[focal_player_used], opp),
  ...
  if (i %% 10 == 0) {
    print(i)
  } # progress of number of games simulated
}
...
beta_draws_list <- lapply(sim_fit_list, function(x) {
  x$draws() |>
    ...
    mutate(
      player_id = stringr::str_extract(name, pattern = "\\d+]"),
...
beta_draws <- beta_draws_list |>
  bind_rows(.id = "dataset_id") |>
  rename(draw = value) |>
  mutate(
    dataset_id = substr(dataset_id, nchar(dataset_id), nchar(dataset_id)),
    dataset_id = as.numeric(dataset_id),
    true_beta = case_when(
      dataset_id %in% 1:3 ~ true_small_betas[player_id],
      dataset_id %in% 4:6 ~ true_med_betas[player_id],
      dataset_id %in% 7:9 ~ true_big_betas[player_id]
    ),
    num_games = case_when(
      dataset_id %in% c(1, 4, 7) ~ 1000,
...
true_betas_plot <- data.frame(
  true_small_val = true_small_betas,
  ...
  mutate(
    ymin = as.numeric(factor(row_number())) - 0.3,
    ymax = as.numeric(factor(row_number())) + 0.3
  )


small_effect_plot <- beta_draws |>
  filter(dataset_id %in% 1:3) |>
  ggplot(aes(y = reorder(player_id, true_beta), x = draw, colour = num_games)) +
  stat_pointinterval(position = ggstance::position_dodgev(height = 0.5)) +
  geom_segment(
    data = true_betas_plot,
...
med_effect_plot <- beta_draws |>
  filter(dataset_id %in% 4:6) |>
  ggplot(aes(y = reorder(player_id, true_beta), x = draw, colour = num_games)) +
...
# plot
big_effect_plot <- beta_draws |>
  filter(dataset_id %in% 7:9) |>
  ggplot(aes(y = reorder(player_id, true_beta), x = draw, colour = num_games)) +
  # stat_histinterval(aes(fill = num_games), position = "dodgejust") +
  stat_pointinterval(position = ggstance::position_dodgev(height = 0.5)) +
  # stat_dots(aes(colour = num_games), position = "dodgejust") +
...
true_mu_betas <- sim_datasets_list |>
  bind_rows() |>
  pull("mu_beta") |>
  unique()

# getting the draws
mu_beta_draws <- mu_beta_draws_list |>
  bind_rows(.id = "dataset_id") |>
  rename(draw = mu_beta) |>
  mutate(
    dataset_id = substr(dataset_id, nchar(dataset_id), nchar(dataset_id)),
    dataset_id = as.numeric(dataset_id),
    true_mu_beta = case_when(
      dataset_id %in% 1:3 ~ true_mu_betas[1],
...
mu_beta_plot <- mu_beta_draws |> ggplot(aes(x = draw, y = true_mu_beta)) +
  ...
  theme_single_y_legend() +
  scale_x_continuous(
    breaks = c(0, signif(true_mu_betas, 3)),
...
```{r figure S3 left}
model_path <- here("results_revision", "lichess1700-1900")

# GM n = 1 model
fit_17_19_bullet_prev <- readRDS(file = here(model_path, "all_rated_bullet_model_n1.RDS"))

# GM n = 5 model
fit_17_19_bullet_5 <- readRDS(file = here(model_path, "all_rated_bullet_model_n5.RDS"))

# GM n = 10 model
fit_17_19_bullet_10 <- readRDS(file = here(model_path, "all_rated_bullet_model_n10.RDS"))

prev_pars <- fit_17_19_bullet_prev$draws(c(
  "mu_beta",
  "gamma1", "gamma2"
)) |>
  as_draws_df() |>
...
pre_5_pars <- fit_17_19_bullet_5$draws(c(
  "mu_beta",
  "gamma1", "gamma2"
)) |>
  as_draws_df() |>
...
pre_10_pars <- fit_17_19_bullet_10$draws(c(
  "mu_beta",
  "gamma1", "gamma2"
)) |>
  as_draws_df() |>
...
# GM n = 5 model
fit_gm_bullet_5 <- readRDS(file = here(model_path, "all_rated_bullet_model_n5.RDS"))

# GM n = 10 model
fit_gm_bullet_10 <- readRDS(file = here(model_path, "all_rated_bullet_model_n10.RDS"))

prev_pars <- fit_gm_bullet_prev$draws(c(
  "mu_beta",
  "gamma1", "gamma2"
)) |>
  as_draws_df() |>
...
pre_5_pars <- fit_gm_bullet_5$draws(c(
  "mu_beta",
  "gamma1", "gamma2"
)) |>
  as_draws_df() |>
...
pre_10_pars <- fit_gm_bullet_10$draws(c(
  "mu_beta",
  "gamma1", "gamma2"
)) |>
  as_draws_df() |>
...
```{r perm for 17-19}
#| eval: false
perm_data_path <- here("results_revision_may/lichess1700-1900/perm_session/")
# res_data_path <-
# path_id <- 1
data_path <- here("results_revision_may/perm/lichess1700-1900/")
...
fit_17_19_bullet_prev <- readRDS(
  file =
    paste0(
      data_path,
      "/all_rated_bullet_model_n1.RDS"
    )
)
...
bullet_17_19 <- readRDS(file = here("results_revision_may", "lichess1700-1900",
                                    "users_bullet.RDS"))
...
player_pars <- sim_pars |>
  as_draws_df() |>
  mutate(
    param = stringr::str_extract(variable, pattern = "\\d"),
...
ggsave(
  plot = perm_17_19_winner,
  filename = paste0(here(
    "Paper_WriteUp", "paper_figures",
    "perm_17_19_winner.png"
  )), dpi = 600,
  height = 5, width = 7
)
```
...
```{r figure 8 right}
perm_data_path <- here("results_revision_may/lichessGrandmasters/perm_session")
data_path <- here("results_revision_may/lichessGrandmasters")
...
perm_gm_global <- sim_pars |>
  ...
  facet_wrap(~variable, scales = "free") +
  geom_vline(
    data = true_values, mapping = aes(xintercept = mean),
    col = "red"
  ) +
  theme_3col() +
  labs(x = element_blank(), y = element_blank())
...
ggsave(
  plot = bul_blit_comp_global,
  filename = paste0(here(
    "Paper_WriteUp", "paper_figures",
    "bul_blitz_comp_17_19_global.png"
  )), dpi = 600,
  height = 5, width = 7
)
...
common_players <- intersect(
  unique(player_pars_bullet$player_name),
...
ggsave(
  plot = comp_17_19_winner,
  filename = paste0(here(
    "Paper_WriteUp", "paper_figures",
    "bul_blitz_comp_17_19_winner.png"
  )), dpi = 600,
  height = 5, width = 7
)
...
ggsave(
  plot = comp_17_19_player,
  filename = paste0(here(
    "Paper_WriteUp", "paper_figures",
    "bul_blitz_comp_17_19_player.png"
  )), dpi = 600,
  height = 5, width = 7
)
```
