...
comp_global = bind_rows(pars_17_19, pars_20_22, pars_23_25, pars_gm) |> 
  mutate(name = factor(name, levels = c("mu_beta", "gamma1", "gamma2"))) |>
...
winner_plot_17_19 <- player_pars |> 
  ...
  scale_x_continuous(breaks = c(-0.06, -0.03, 0, 0.03, 0.06), 
                     labels = c(-0.06, -0.03, 0, 0.03, 0.06),
                     limits = c(-0.062, 0.062)) +
  theme_single_y()
...
med_effect_plot = beta_draws %>% filter(dataset_id %in% 4:6) %>%
  ...
  geom_vline(aes(xintercept = 0), col = "red", linetype = "dashed") +
  theme_single_y_legend() +
  scale_color_viridis_d(option = "D", begin = 0, end = 0.75)
...
```{r perm for 17-19}
#| eval: false
perm_data_path <- here("results_revision/lichess1700-1900/perm")
...
data_path <- here("results_revision/lichess1700-1900")

fit_17_19_bullet_prev <- readRDS(file = paste0(data_path, "/all_rated_bullet_model_n1.RDS"))
...
```{r figure 8 right}
data_path <- here("results_revision/lichessGrandmasters/")
perm_data_path <- here("results_revision/lichessGrandmasters/perm/")
...
fit_gm_bullet_prev <- readRDS(file = paste0(data_path, "/all_rated_bullet_model_n1.RDS"))
...
perm_gm_winner <- player_pars |> 
  ...
  geom_segment(data = true_vals,
               aes(x = mean, y = y_fake, yend = yend), col = "red") +
  theme_single_y() +
  labs(x = "Estimated W/L-Effect", y = "") + xlim(c(-1,1))
