...
bul_blit_gm_comp_global <- bind_rows(bullet_pars, blitz_pars) |>
  ...
  theme(legend.position = "bottom") +
  facetted_pos_scales(x = list( #for customizing scales on each facet
    name == "mu_beta" ~ scale_x_continuous(breaks = c(0, 0.5), limits = c(0, 0.6)),
    name == "gamma1" ~ scale_x_continuous(breaks = breaks_pretty(n = 2)),
    name == "gamma2" ~ scale_x_continuous(breaks = breaks_pretty(n = 2), limits = c(0.005, 0.0057))
  )) +
  #scale_x_continuous(breaks = breaks_pretty(n = 2)) +
  scale_color_viridis_d(option = "H")
...
### SET FIRST RD, VOLATILITY TO TRUE RD/VOLATILITY WE GET AFTER FINDING RDS/VOLATILITY IN RAW DATASET (IN INIT CALCULATION)
...
#distribution of x_{ij} - for response to reviewer comment
```{r}
#import GM data
