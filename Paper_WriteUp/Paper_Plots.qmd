...
comp_global = bind_rows(pars_17_19, pars_20_22, pars_23_25, pars_gm) |> 
  ...
  facetted_pos_scales(x = list( #for customising scales on each facet
    name == "mu_beta" ~ scale_x_continuous(breaks = c(-0.15, 0, 0.15), limits = c(-0.17, 0.17)),
...
true_mu_betas_plotting = data.frame(true_val = true_mu_betas) %>%
  mutate(ymin = as.numeric(as.factor(true_val)) - 0.3,
         ymax = as.numeric(as.factor(true_val)) + 0.3)
