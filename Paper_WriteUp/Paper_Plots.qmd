...
#read in models
model_17_19 = readRDS(here("results_revision", "lichess1700-1900", "all_rated_bullet_model_prev_n1.RDS"))
model_20_22 = readRDS(here("results_revision", "lichess2000-2200", "all_rated_bullet_model_prev_n1.RDS"))
model_23_25 = readRDS(here("results_revision", "lichess2300-2500", "all_rated_bullet_model_prev_n1.RDS"))
model_gm = readRDS(here("results_revision", "lichessGrandmasters", "all_rated_bullet_model_prev_n1.RDS"))


#get global effects of each
pars_17_19 <- model_17_19$draws(c("mu_beta", "gamma1", "gamma2")) |> 
  as_draws_df() |> 
  pivot_longer(cols = "mu_beta":"gamma2") |> 
  mutate(model = "1700-1900")

pars_20_22 = model_20_22$draws(c("mu_beta", "gamma1", "gamma2")) |> 
  as_draws_df() |> 
  pivot_longer(cols = "mu_beta":"gamma2") |> 
  mutate(model = "2000-2200")

pars_23_25 = model_23_25$draws(c("mu_beta", "gamma1", "gamma2")) |> 
  as_draws_df() |> 
  pivot_longer(cols = "mu_beta":"gamma2") |> 
  mutate(model = "2300-2500")

pars_gm <- model_gm$draws(c("mu_beta", "gamma1", "gamma2")) |> 
  as_draws_df() |> 
  pivot_longer(cols = "mu_beta":"gamma2") |> 
  mutate(model = "GM")
...
gm_amateur_comp_global <- bind_rows(pars_17_19, pars_20_22, pars_23_25, pars_gm) |> 
...
model = readRDS(file = here(model_path, "all_rated_bullet_model_prev_n1.RDS"))
...
init_data = tidy_games |>
  ...
  dplyr::select(focal_user, focal_id, focal_white, WhiteElo, BlackElo,
...
for (i in 1:num_games) {
  ...
  rating_diff = curr_game_info$elo_diff
  ...
  if (i == 1) {
    ...
    curr_hist = 0.5 #assume they're current past performance is at their usual level
  } else {
  ...
  #store current result and rating
...
global_plot = sim_fit_list[[sim_dataset_num]]$draws(c("mu_beta", "gamma1", "gamma2", 
                                                      "sigma_1", "tau[1]", "tau[2]", 
                                                      "sigma_g1", "sigma_g2")) |>
  as_draws_df() |>
  pivot_longer(cols = "mu_beta":"sigma_g2") |>
...
#order of min to max estiamted betas - makes plot look nicer
beta_order = sim_datasets_list[[1]] %>% group_by(player_id) %>% summarise(beta = unique(beta)) %>% arrange(beta) %>% pull(player_id)
...
#combing fit results by mu_beta size
small_effect_draws = mu_beta_draws_list[1:3] %>% bind_rows(.id = "column_label") %>% 
  mutate(num_games = case_when(endsWith(column_label, "1") ~ 1000,
                               endsWith(column_label, "2") ~ 5000,
                               endsWith(column_label, "3") ~ 20000),
         num_games = factor(num_games, levels = c("1000", "5000", "20000"))) #make sure the levels are messing up here
  
med_effect_draws = mu_beta_draws_list[4:6] %>% bind_rows(.id = "column_label") %>% 
  mutate(num_games = case_when(endsWith(column_label, "4") ~ 1000,
                               endsWith(column_label, "5") ~ 5000,
                               endsWith(column_label, "6") ~ 20000),
         num_games = factor(num_games, levels = c("1000", "5000", "20000"))) #make sure the levels are messing up here

big_effect_draws = mu_beta_draws_list[7:9] %>% bind_rows(.id = "column_label") %>% 
  mutate(num_games = case_when(endsWith(column_label, "7") ~ 1000,
                               endsWith(column_label, "8") ~ 5000,
                               endsWith(column_label, "9") ~ 20000),
         num_games = factor(num_games, levels = c("1000", "5000", "20000"))) #make sure the levels are messing up here
